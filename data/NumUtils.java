...
/**
 * This class provides a few static functions for converting real numbers to
 * strings and strings to real numbers. It should probably be reimplemented to
 * use the standard NumberFormat class for converting real to string.
 */
public class NumUtils {

	/**
	 * Return the real number represented by the String s, or return Double.NaN
	 * if s does not represent a legal real number.
	 */
	...
	/**
	 * Return a string representation of the real number x occupying, if
	 * possible, at most 10 spaces.
	 */
	...
	/**
	 * Goal is to return a reasonable string representation of x, using at most
	 * width spaces. (If the parameter width is unreasonably big or small, its
	 * value is adjusted to lie in the range 6 to 25.)
	 *
	 * @param x
	 *            value to create string representation of.
	 * @param width
	 *            maximum number of spaces used in string representation, if
	 *            possible.
	 * @return a string representation for x. If x is Double.NaN, "undefined" is
	 *         returned. If x is infinite, "INF" or "-INF" is returned.
	 */
	public static String realToString(double x, int width) {
		...
		if (Math.rint(x) == x && Math.abs(x) < 5e15
				&& s.length() <= (width + 2))
			return String.valueOf((long) x); // return string without trailing
												// ".0"
		if (s.length() <= width)
			return s;
		boolean neg = false;
